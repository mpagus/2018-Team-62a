...
#pragma config(Motor,  port2,           ripperR,       tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port3,           ripperL,       tmotorVex393HighSpeed_MC29, openLoop, reversed)
...
task siveMotors(){
	...
	while(true){
		if(vexRT[Btn6U] && !sive){
			...
			siveMotor(0);
		else if(!sive && SensorValue(sivePot) < 3000)
			siveMotor(-127);
		else
			siveMotor(0);
...
void normalStackCone(int cone){
	...
	wait1Msec(0+250+32*cone);
	desiredBar = 3250; //four bar outtake
	wait1Msec(0+300+28*cone);//this is the fucked one
	stopTask(ripperControl);
	ripper(-20);
	wait1Msec(550);
	desiredBar = 2650; //intakeUp
	wait1Msec(200);
	ripper(0);
	//wait1Msec(200);
...
/*	drive(-30,-30);
	...
	wait1Msec(2000);
	drive(85,80);
	wait1Msec(680);//drive back to bar
	drive(60,0);//turn
	wait1Msec(2100);
	drive(-80,-80);
	wait1Msec(1200);
...
*/
	...
	wait1Msec(1500);
	drive(65,60);
	wait1Msec(300);//drive back to bar
	drive(0,100);//turn
	wait1Msec(1300);
	drive(30,30);
	wait1Msec(200);
