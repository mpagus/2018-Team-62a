...
void moveStage1WaitUntil(float desiredValue, float continueValue){
	...
	moveSingleStageWaitUntil(stage1Encoder, continueValue, 40);
}
...
void moveStage2WaitUntil(float desiredValue, float continueValue){
	...
	moveSingleStageWaitUntil(stage2Encoder, continueValue+numRevolutions*ticksPerRevolution + 318, 60);
}
...
void stage2Revolution(){
	numRevolutions = numRevolutions+1;
	resetSlewStage2 = true;
	moveSingleStageWait(stage1Encoder, desiredStage2+numRevolutions*ticksPerRevolution - stage2Scalar, 60, 60);
}
...
void highStackCone(float cone){
	moveBothStagesWait(stage1FormulaHighGoal(19 + 2.5*cone), stage2FormulaHighGoal(16 + 2.5*cone));
}
...
task mobileGoalMotors(){
	...
	while(true){
		if(mobileTip){
			if(mobileGoalTip){
				if(SensorValue(intakeEncoder) < 1860 || SensorValue(intakeEncoder) > 1940)
			...
			if(SensorValue(intakeEncoder) < 2650 || SensorValue(intakeEncoder) > 2750)
				intake(-0.15*(SensorValue(intakeEncoder)-2700));
			else
				intake(5);
		}
		else if(!mobileGoal){
			if(SensorValue(intakeEncoder) < 3400)
				intake(127);
			else if(SensorValue(intakeEncoder) < 3600)
				intake(30);
			else
				intake(5);
		}
...
void mobileGoalOut(){
	mobileGoal = false;
	while(SensorValue(intakeEncoder) < 3550){
		wait1Msec(80);
	}
}
...
void mobileGoalIn(){
	mobileGoal = true;
	while(SensorValue(intakeEncoder) > 1800){
		wait1Msec(80);
	}
	wait1Msec(300);
}
...
#include "Team 62 Mark V Skills.c"
//#include "Team 62 Mark V Match.c"
